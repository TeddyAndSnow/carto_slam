...
namespace carto_slam
{
  namespace common
  {
    ...
    public:
      ...
      void Trim(const estimator::MapById<estimator::NodeId, NodeType> &nodes,
                const estimator::NodeId &node_id)
      {
      ...
      estimator::Range<ConstTrajectoryIterator> trajectory_ids() const
      {
        return estimator::Range<ConstTrajectoryIterator>(
            ConstTrajectoryIterator(data_.begin()),
            ConstTrajectoryIterator(data_.end()));
      }

      estimator::Range<ConstIterator> trajectory(const int trajectory_id) const
      {
        return estimator::Range<ConstIterator>(BeginOfTrajectory(trajectory_id),
