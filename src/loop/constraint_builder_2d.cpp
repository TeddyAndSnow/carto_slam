...
namespace carto_slam
{
  namespace loop
  {
    ...
    common::Rigid2d ComputeSubmapPose(const Submap2D &submap)
    {
      return common::Project2D(submap.local_pose());
    }

    ConstraintBuilder2D::ConstraintBuilder2D(const Config &options, common::ThreadPoolInterface *const thread_pool)
        : options_(options),
          thread_pool_(thread_pool),
          finish_node_task_(std::make_unique<common::Task>()),
          when_done_task_(std::make_unique<common::Task>()),
          ceres_scan_matcher_(options) {}
        ...
        const common::Rigid2d &initial_relative_pose)
    {
      if (initial_relative_pose.translation().norm() >
          options_.constraint_builder_max_constraint_distance_)
      {
        return;
      }
      if (!per_submap_sampler_
               .emplace(std::piecewise_construct, std::forward_as_tuple(submap_id),
                        std::forward_as_tuple(options_.constraint_builder_sampling_ratio_))
               .first->second.Pulse())
      {
        return;
      }
        ...
        const NodeId &node_id, const TrajectoryNode::Data *const constant_data)
    {
      ...
      constraint_task->SetWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
        ComputeConstraint(submap_id, submap, node_id, true, /* match_full_submap */
                          constant_data, common::Rigid2d::Identity(),
      ...
      scan_matcher_task->SetWorkItem(
          [&submap_scan_matcher, &scan_matcher_options]() {
            submap_scan_matcher.fast_correlative_scan_matcher =
                std::make_unique<FastCorrelativeScanMatcher2D>(
        ...
        std::unique_ptr<ConstraintBuilder2D::Constraint> *constraint)
    {
      ...
      const common::Rigid2d initial_pose =
      ...
      float score = 0.;
      common::Rigid2d pose_estimate = common::Rigid2d::Identity();
      ...
      if (match_full_submap)
      {
        ...
        if (submap_scan_matcher.fast_correlative_scan_matcher->MatchFullSubmap(
                ...
                options_.constraint_builder_global_localization_min_score_, &score, &pose_estimate))
        {
          // CHECK_GT(score, options_.constraint_builder_global_localization_min_score_);
      ...
      else
      {
        ...
        if (submap_scan_matcher.fast_correlative_scan_matcher->Match(
                ...
                options_.constraint_builder_min_score_, &score, &pose_estimate))
        {
          // We've reported a successful local match.
          // CHECK_GT(score, options_.constraint_builder_min_score_);
      ...
      constraint->reset(new Constraint{submap_id,
                                       ...
                                       {common::Embed3D(constraint_transform),
                                        options_.constraint_builder_loop_closure_translation_weight_,
                                        options_.constraint_builder_loop_closure_rotation_weight_},
      ...
      if (options_.constraint_builder_log_matches_)
      {
        ...
        {
          const common::Rigid2d difference =
