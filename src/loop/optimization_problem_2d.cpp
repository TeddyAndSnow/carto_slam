...
#include "loop/landmark_cost_function_2d.h"
#include "loop/spa_cost_function_2d.h"
...
namespace carto_slam
{
  ...
  namespace loop
  {
    namespace
    {
      ...
      using loop::CeresPose;
      using LandmarkNode = loop::PoseGraphInterface::LandmarkNode;
      using TrajectoryData = loop::PoseGraphInterface::TrajectoryData;

      // For fixed frame pose.
      std::unique_ptr<common::Rigid3d> Interpolate(const common::MapByTime<common::FixedFramePoseData> &map_by_time,
                                                   const int trajectory_id, const common::Time time)
      {
        ...
        if (it == map_by_time.BeginOfTrajectory(trajectory_id))
        {
          if (it->time == time)
          {
            return std::make_unique<common::Rigid3d>(it->pose.value());
          }
          return nullptr;
        }
        ...
        {
          return std::make_unique<common::Rigid3d>(
              Interpolate(common::TimestampedTransform{prev_it->time,
                                                       prev_it->pose.value()},
                          common::TimestampedTransform{it->time, it->pose.value()},
                          time)
                  .transform);
        }
        return nullptr;
      ...
      // orientation.
      std::array<double, 3> FromPose(const common::Rigid2d &pose)
      {
      ...
      common::Rigid2d ToPose(const std::array<double, 3> &values)
      {
        return common::Rigid2d({values[0], values[1]}, values[2]);
      }

      // Selects a trajectory node closest in time to the landmark observation and
      // applies a relative transform from it.
      common::Rigid3d GetInitialLandmarkPose(const LandmarkNode::LandmarkObservation &observation,
                                             ...
                                             const std::array<double, 3> &next_node_pose)
      {
        ...
        return common::Rigid3d::FromArrays(std::get<0>(rotation_and_translation),
                                    ...
                                    double huber_scale)
      {
        for (const auto &landmark_node : landmark_nodes)
        {
          for (const auto &observation : landmark_node.second.landmark_observations)
          {
            ...
            {
              const common::Rigid3d starting_point =
    ...
    OptimizationProblem2D::OptimizationProblem2D(const Config &options)
        : options_(options) {}
    ...
    void OptimizationProblem2D::AddOdometryData(
        const int trajectory_id, const common::OdometryData &odometry_data)
    {
    ...
    void OptimizationProblem2D::AddSubmap(
        const int trajectory_id, const common::Rigid2d &global_submap_pose)
    {
      submap_data_.Append(trajectory_id, SubmapSpec2D{global_submap_pose});
    }

    void OptimizationProblem2D::InsertSubmap(
        const SubmapId &submap_id, const common::Rigid2d &global_submap_pose)
    {
    ...
    {
      options_.optimization_problem_max_num_iterations_ = max_num_iterations;
    }
        ...
        const std::map<std::string, LandmarkNode> &landmark_nodes)
    {
      ...
      for (auto node_it = node_data_.begin(); node_it != node_data_.end();)
      {
        ...
        for (++node_it; node_it != trajectory_end; ++node_it)
        {
          ...
          std::unique_ptr<common::Rigid3d> relative_odometry =
          ...
          if (relative_odometry != nullptr)
          {
            problem.AddResidualBlock(
                CreateAutoDiffSpaCostFunction(Constraint::Pose{
                    *relative_odometry, options_.odometry_translation_weight_,
                    options_.odometry_rotation_weight_}),
                nullptr /* loss function */, C_nodes.at(first_node_id).data(),
                C_nodes.at(second_node_id).data());
          }

          // Add a relative pose constraint based on consecutive local SLAM poses.
          const common::Rigid3d relative_local_slam_pose =
          ...
          problem.AddResidualBlock(
              CreateAutoDiffSpaCostFunction(
                  Constraint::Pose{relative_local_slam_pose,
                                   options_.local_slam_pose_translation_weight_,
                                   options_.local_slam_pose_rotation_weight_}),
              nullptr /* loss function */, C_nodes.at(first_node_id).data(),
              C_nodes.at(second_node_id).data());
        }
      }
      ...
      for (auto node_it = node_data_.begin(); node_it != node_data_.end();)
      {
        ...
        for (; node_it != trajectory_end; ++node_it)
        {
          ...
          const std::unique_ptr<common::Rigid3d> fixed_frame_pose =
          ...
          const Constraint::Pose constraint_pose{
              *fixed_frame_pose, options_.fixed_frame_pose_translation_weight_,
              options_.fixed_frame_pose_rotation_weight_};

          if (!fixed_frame_pose_initialized)
          {
            common::Rigid2d fixed_frame_pose_in_map;
            if (trajectory_data.fixed_frame_origin_in_map.has_value())
            {
              fixed_frame_pose_in_map = common::Project2D(
            ...
            else
            {
              fixed_frame_pose_in_map =
                  ...
                  common::Project2D(constraint_pose.zbar_ij).inverse();
            }
          ...
          problem.AddResidualBlock(
              ...
              options_.fixed_frame_pose_use_tolerant_loss_
                  ? new ceres::TolerantLoss(
                        options_.fixed_frame_pose_tolerant_loss_param_a_,
                        options_.fixed_frame_pose_tolerant_loss_param_b_)
                  : nullptr,
      ...
      options.use_nonmonotonic_steps = options_.optimization_problem_use_nonmonotonic_steps_;
      options.max_num_iterations = options_.optimization_problem_max_num_iterations_;
      options.num_threads = options_.optimization_problem_num_threads_;
      ...
      for (const auto &C_fixed_frame : C_fixed_frames)
      {
        trajectory_data_.at(C_fixed_frame.first).fixed_frame_origin_in_map =
            common::Embed3D(ToPose(C_fixed_frame.second));
      }
    ...
    std::unique_ptr<common::Rigid3d> OptimizationProblem2D::InterpolateOdometry(
        const int trajectory_id, const common::Time time) const
    {
      ...
      if (it == odometry_data_.BeginOfTrajectory(trajectory_id))
      {
        if (it->time == time)
        {
          return std::make_unique<common::Rigid3d>(it->pose);
        }
        return nullptr;
      }
      const auto prev_it = std::prev(it);
      return std::make_unique<common::Rigid3d>(
          Interpolate(common::TimestampedTransform{prev_it->time, prev_it->pose},
                      common::TimestampedTransform{it->time, it->pose}, time)
              .transform);
    }

    std::unique_ptr<common::Rigid3d>
        ...
        const NodeSpec2D &second_node_data) const
    {
      ...
      {
        const std::unique_ptr<common::Rigid3d> first_node_odometry =
            InterpolateOdometry(trajectory_id, first_node_data.time);
        const std::unique_ptr<common::Rigid3d> second_node_odometry =
            InterpolateOdometry(trajectory_id, second_node_data.time);
        if (first_node_odometry != nullptr && second_node_odometry != nullptr)
        {
          common::Rigid3d relative_odometry =
              common::Rigid3d::Rotation(first_node_data.gravity_alignment) *
              first_node_odometry->inverse() * (*second_node_odometry) *
              common::Rigid3d::Rotation(
                  second_node_data.gravity_alignment.inverse());
          return std::make_unique<common::Rigid3d>(relative_odometry);
        }
      }
      return nullptr;
    }
