...
#include "loop/overlapping_submaps_trimmer_2d.h"
// #include "cartographer/mapping/proto/pose_graph/constraint_builder_options.pb.h"
#include "common/compressed_point_cloud.h"
...
namespace carto_slam
{
    namespace loop
    {
        ...
        PoseGraph2D::PoseGraph2D(
            ...
            std::unique_ptr<OptimizationProblem2D> optimization_problem,
            common::ThreadPool *thread_pool)
            : options_(options),
              optimization_problem_(std::move(optimization_problem)),
              constraint_builder_(options_, thread_pool),
            ...
            const std::vector<std::shared_ptr<const Submap2D>> &insertion_submaps)
        {
            ...
            if (insertion_submaps.size() == 1)
            {
                ...
                if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0)
                {
                    ...
                    optimization_problem_->AddSubmap(
                        trajectory_id, common::Project2D(
                ...
                insertion_submaps.front())
            {
                ...
                optimization_problem_->AddSubmap(
                    trajectory_id,
                    first_submap_pose *
                        ComputeSubmapPose(*insertion_submaps[0]).inverse() *
            ...
            if (!global_localization_samplers_[trajectory_id])
            {
                global_localization_samplers_[trajectory_id] =
                    absl::make_unique<common::FixedRatioSampler>(
                        options_.pose_graph_global_sampling_ratio_);
            }
        }
        ...
        void PoseGraph2D::AddLandmarkData(int trajectory_id,
                                          const common::LandmarkData &landmark_data)
        {
            AddWorkItem([=]() LOCKS_EXCLUDED(mutex_) {
                absl::MutexLock locker(&mutex_);
                if (CanAddWorkItemModifying(trajectory_id))
                                            ...
                                            const SubmapId &submap_id)
        {
            ...
            {
                const common::Rigid2d initial_relative_pose =
            ...
            const bool newly_finished_submap)
        {
            ...
            std::set<NodeId> newly_finished_submap_node_ids;
            {
                ...
                const common::Rigid2d local_pose_2d =
                    common::Project2D(constant_data->local_pose *
                                         common::Rigid3d::Rotation(
                                             constant_data->gravity_alignment.inverse()));
                const common::Rigid2d global_pose_2d =
                    optimization_problem_->submap_data().at(matching_id).global_pose *
                    ComputeSubmapPose(*insertion_submaps.front()).inverse() *
                    local_pose_2d;
                optimization_problem_->AddTrajectoryNode(
                    matching_id.trajectory_id,
                    NodeSpec2D{constant_data->time, local_pose_2d,
                                             global_pose_2d,
                ...
                for (size_t i = 0; i < insertion_submaps.size(); ++i)
                {
                    ...
                    const common::Rigid2d constraint_transform =
                        ComputeSubmapPose(*insertion_submaps[i]).inverse() *
                        local_pose_2d;
                    data_.constraints.push_back(
                        Constraint{submap_id,
                                   node_id,
                                   {common::Embed3D(constraint_transform),
                                    options_.pose_graph_matcher_translation_weight_,
                                    options_.pose_graph_matcher_rotation_weight_},
            ...
            if (options_.pose_graph_optimize_every_n_nodes_ > 0 &&
                num_nodes_since_last_loop_closure_ > options_.pose_graph_optimize_every_n_nodes_)
            {
        ...
        void PoseGraph2D::HandleWorkQueue(
            const ConstraintBuilder2D::Result &result)
        {
            {
                absl::MutexLock locker(&mutex_);
...

            {
                ...
                {
                    if (constraint.tag ==
                        loop::PoseGraph::Constraint::INTRA_SUBMAP)
                    {
                        continue;
                    }
        ...
        void PoseGraph2D::DrainWorkQueue()
        {
            ...
            constraint_builder_.WhenDone(
                [this](const ConstraintBuilder2D::Result &result) {
        ...
        void PoseGraph2D::WaitForAllComputations()
        {
            ...
            constraint_builder_.WhenDone(
                [this,
                 &notification](const loop::ConstraintBuilder2D::Result &result)
        ...
        // void PoseGraph2D::AddSubmapFromProto(const common::Rigid3d &global_submap_pose, const proto::Submap &submap)
        // {
        ...
        //     const common::Rigid2d global_submap_pose_2d =
        ...
        // void PoseGraph2D::AddNodeFromProto(const common::Rigid3d &global_pose, const proto::Node &node)
        // {
        ...
        //         const auto &constant_data =
        //             data_.trajectory_nodes.at(node_id).constant_data;
        //         const auto gravity_alignment_inverse = common::Rigid3d::Rotation(
        //             constant_data->gravity_alignment.inverse());
        ...
        //                 common::Project2D(global_pose * gravity_alignment_inverse),
        //                 constant_data->gravity_alignment});
        //         return WorkItem::Result::kDoNotRunOptimization;
        //     });
        // }
        ...
        //             common::ToRigid3(data.fixed_frame_origin_in_map());
        ...
        {
            {
                AddWorkItem([this]() LOCKS_EXCLUDED(mutex_) {
                    ...
                    optimization_problem_->SetMaxNumIterations(
                        options_.pose_graph_max_num_final_iterations_);
                    return WorkItem::Result::kRunOptimization;
                });
                AddWorkItem([this]() LOCKS_EXCLUDED(mutex_) {
                    absl::MutexLock locker(&mutex_);
                    optimization_problem_->SetMaxNumIterations(
                        options_.optimization_problem_ceres_solver_options_max_num_iterations_);
                    return WorkItem::Result::kDoNotRunOptimization;
                });
            }
            WaitForAllComputations();
            ...
            for (const int trajectory_id : node_data.trajectory_ids())
            {
                for (const auto &node : node_data.trajectory(trajectory_id))
                {
                    ...
                    mutable_trajectory_node.global_pose =
                        common::Embed3D(node.data.global_pose_2d) *
                        common::Rigid3d::Rotation(
                ...
                const auto local_to_old_global = ComputeLocalToGlobalTransform(
                    data_.global_submap_poses_2d, trajectory_id);
                const common::Rigid3d old_global_to_new_global =
        ...
        std::map<std::string, common::Rigid3d> PoseGraph2D::GetLandmarkPoses()
            const
        {
            std::map<std::string, common::Rigid3d> landmark_poses;
            absl::MutexLock locker(&mutex_);
        ...
        common::MapByTime<common::ImuData> PoseGraph2D::GetImuData() const
        {
            absl::MutexLock locker(&mutex_);
            return optimization_problem_->imu_data();
        }

        common::MapByTime<common::OdometryData> PoseGraph2D::GetOdometryData() const
        {
            absl::MutexLock locker(&mutex_);
        ...
        common::MapByTime<common::FixedFramePoseData>
        ...
        common::Rigid3d PoseGraph2D::GetInterpolatedGlobalTrajectoryPose(
            const int trajectory_id, const common::Time time) const
        {
            ...
            return common::Interpolate(
                       common::TimestampedTransform{std::prev(it)->data.time(),
                                                       std::prev(it)->data.global_pose},
                       common::TimestampedTransform{it->data.time(),
                                                       it->data.global_pose},
                       time)
                .transform;
        }

        common::Rigid3d PoseGraph2D::GetLocalToGlobalTransform(
        ...
        common::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
            const MapById<SubmapId, SubmapSpec2D> &global_submap_poses,
            const int trajectory_id) const
        {
            ...
            if (begin_it == end_it)
            {
                ...
                {
                    return common::Rigid3d::Identity();
                }
            }
            ...
            for (const auto &submap_id_data : parent_->data_.submap_data)
            {
                ...
                submaps.Insert(
                    ...
                    SubmapData{submap_id_data.data.submap,
                               common::Embed3D(parent_->data_.global_submap_poses_2d
