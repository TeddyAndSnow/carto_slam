...
namespace carto_slam
{
  namespace io
  {
    namespace
    {
      ...
      Eigen::Affine3d ToEigen(const common::Rigid3d &rigid3)
      {
                                            ...
                                            const int width, const int height)
    {
      ...
      std::string cells;
      FastGunzipString(compressed_cells, &cells);
                                      ...
                                      std::vector<uint32_t> *const cairo_data)
    {
      ...
      // CHECK_EQ(cairo_surface_status(surface.get()), CAIRO_STATUS_SUCCESS) << cairo_status_to_string(cairo_surface_status(surface.get()));
      return surface;
    }
