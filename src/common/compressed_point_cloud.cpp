...
namespace carto_slam
{
  namespace common
  {
        ...
        : num_points_(point_cloud.size())
    {
      ...
      using Blocks = estimator::HybridGridBase<std::vector<RasterPoint>>;
           ...
           ++point_index)
      {
        ...
        // CHECK_LT(point.position.cwiseAbs().maxCoeff() / kPrecision, 1 << kMaxBitsPerDirection) << "Point out of bounds: " << point.position;
