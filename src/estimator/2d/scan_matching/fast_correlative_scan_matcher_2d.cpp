...
#include "estimator/2d/scan_matching/fast_correlative_scan_matcher_2d.h"
...
namespace carto_slam
{
  namespace estimator
  {
    ...
    {
      CHECK_GE(options.fast_correlative_scan_matcher_branch_and_bound_depth_, 1);
      const int max_width = 1 << (options.fast_correlative_scan_matcher_branch_and_bound_depth_ - 1);
      precomputation_grids_.reserve(options.fast_correlative_scan_matcher_branch_and_bound_depth_);
      std::vector<float> reusable_intermediate_grid;
      const CellLimits limits = grid.limits().cell_limits();
      reusable_intermediate_grid.reserve((limits.num_x_cells + max_width - 1) *
                                         limits.num_y_cells);
      for (int i = 0; i != options.fast_correlative_scan_matcher_branch_and_bound_depth_; ++i)
      {
        const int width = 1 << i;
    ...
    bool FastCorrelativeScanMatcher2D::Match(
        ...
        common::Rigid2d *pose_estimate) const
    {
      const SearchParameters search_parameters(options_.fast_correlative_scan_matcher_linear_search_window_,
                                               options_.fast_correlative_scan_matcher_angular_search_window_,
        ...
        common::Rigid2d *pose_estimate) const
    {
      ...
      const SearchParameters search_parameters(
          ...
          point_cloud, limits_.resolution());
      const common::Rigid2d center = common::Rigid2d::Translation(
        ...
        common::Rigid2d *pose_estimate) const
    {
      ...
      const common::PointCloud rotated_point_cloud = common::TransformPointCloud(
          point_cloud,
          common::Rigid3f::Rotation(Eigen::AngleAxisf(
