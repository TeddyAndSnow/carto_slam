...
#include "estimator/2d/scan_matching/correlative_scan_matcher_2d.h"
...
namespace carto_slam
{
  namespace estimator
  {
        ...
        : resolution(resolution)
    {
      ...
      for (const common::RangefinderPoint &point : point_cloud)
      {
        ...
        const SearchParameters &search_parameters)
    {
               ...
               delta_theta += search_parameters.angular_perturbation_step_size)
      {
        rotated_scans.push_back(common::TransformPointCloud(
            point_cloud, common::Rigid3f::Rotation(Eigen::AngleAxisf(
    ...
    std::vector<DiscreteScan2D> DiscretizeScans(
        const MapLimits &map_limits, const std::vector<common::PointCloud> &scans,
        const Eigen::Translation2f &initial_translation)
    {
      ...
      for (const common::PointCloud &scan : scans)
      {
        ...
        for (const common::RangefinderPoint &point : scan)
        {
